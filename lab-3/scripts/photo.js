 ...
 for (var j=0;j<photos.length;j++){
  if(j==0){
   ...
   line+='</tr><tr>';
		...
		line+=
   '<td onclick="showModal('+j+')">'+
    '<a class="linkImg">'+
     '<img src="'+photos[j]+'" '+
     'alt="'+photoTitles[j]+'"'+
     '>'+
     '<p class="title">'+photoTitles[j]+'</p>'+
    '</a>'+
    '</td>';
	}
 line+='</tr></table>'
...
function showModal(j){
	...
	'<a href="" id="closemodal" onclick="closeModal()">X</a>'+
     '<img id="imgmodal" src="'+photos[j]+'">'+
     '<p id="titlemodal">'+photoTitles[j]+'</p>';
